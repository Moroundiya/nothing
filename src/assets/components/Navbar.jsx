...
export default function Navbar({ callback }) {
    ...
    return (
        <div className='w-full h-auto bg-white py-4 sm:py-5 px-4 md:px-8 flex items-center justify-between backdrop-blur-xl fixed left-0 top-0 z-50 lg:px-16 lg:py-8'>
            <h1 className='font-[Ndot] text-[1.4rem] sm:text-2xl lg:text-3xl'>NOTHING</h1>
            ...
            <div className='block md:hidden' onClick={() => {
                ...
                {getmobile ? <CgClose className='tab-parent text-[1.7rem] text-[#000] font-thin cursor-pointer' /> : <CgMenuRightAlt className='tab-parent text-[1.75rem] sm:text-3xl text-[#000] font-thin cursor-pointer' />}
            </div>
        </div >
    )
}
