...
export default function Header() {
    ...
    return (
        <div className='w-full h-full md:h-[600px] lg:h-[700px] xl:h-[850px] 2xl:h-full bg-header-bg bg-center bg-cover bg-no-repeat'>
            <div className={grabValue ? 'bg-white fixed top-[65px] sm:top-[68px] border-t border-b-4 left-0 h-auto pb-8 w-full transition-all z-30 duration-700 ease-in-out md:hidden' :
                'bg-white fixed -top-[230px] border-t border-b-4 left-0 h-auto w-full transition-all z-30 pb-8 duration-700 ease-in-out'}>
                <div className='text:[1.08rem] sm:text-[1.1rem] font-[Figtree] text-[#000] pt-5 px-4 space-y-3'>
                ...
                <div className='text-[1.35rem] sm:text-[1.4rem] space-x-4 flex px-4 mt-8'>
            ...
            <div className='w-full h-full mt-32 lg:mt-48 text-center'>
                <h1 className='font-[Ndot] text-[1.7rem] sm:text-[2.1rem] md:text-5xl lg:text-[3.5rem] xl:text-7xl semi-bold text-black' data-aos="fade-down">NOTHING PHONE (2)</h1>
                <p className='font-[Figtree] leading-snug px-4 text-[1.1rem] sm:text-lg lg:text-[1.35rem] lg:w-2/4 xl:text-2xl my-5 md:w-3/5 2xl:w-3/6 mx-auto text-[#3d3d3d]' data-aos="zoom-in">A new era. Where iconic design meets premium performance. Come to the bright side.
                    Phone (2) available in select drops on 15 July 2023.</p>
                <a href='#' className='font-[Figtree] text-lg sm:text-xl md:text-xl xl:text-2xl mt-6 font-semibold hover:text-[#0f0e0e] block' data-aos="fade-up">View models</a>
            </div>
        </div>
    )
}
