    ...
    return (
        <div className='w-full h-full cursor-pointer relative overflow-hidden group rounded-[40px] md:w-full md:h-11/12' data-aos={props.animate}>
            ...
            <div className='w-full h-full absolute top-0 left-0 px-4 pb-10 lg:px-10 lg:pb-16 flex flex-col justify-end transition-all duration-300'>
                <h1 className='font-[Ndot] text-3xl lg:text-[2.6rem] text-white mb-3 font-thin transition-all duration-1000 ease-in-out stroke-black'>{props.title}</h1>
                <p className='text-white text-lg leading-snug lg:text-2xl font-[Figtree] font-light transition-all duration-[.5s] left-0 top-full hidden group-hover:block'>{props.content}</p>
            </div>
        </div>
    )
}
